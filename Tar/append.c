...
{
    bool is_equal_name = my_strcmp(header.name, list->value.name) == 0;
    int origin_time = number_from_oct(header.mtime);
    int for_replace_time = number_from_oct(list->value.mtime);

    while (list != NULL)
    {
        if (is_equal_name && for_replace_time - origin_time > 0)
        {
            list->should_add = true;
        }
        list = list->next;
    }
}
